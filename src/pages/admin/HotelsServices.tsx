...
export function HotelsServices() {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
          <div className="border-b border-gray-100 px-5 py-3">
            <h3 className="text-sm font-semibold text-gray-900">Homes & Stays Management</h3>
          </div>
          <div className="p-5">
  ...
  // Filter services to only show Hotels
  const hotelsServices = services.filter(service =>
    service.service_categories?.name?.toLowerCase() === 'hotels'
  );

  const pendingServices = hotelsServices.filter(service => service.status === 'pending');
  const approvedServices = hotelsServices.filter(service => service.status === 'approved');
  const rejectedServices = hotelsServices.filter(service => service.status === 'rejected');

  // Filter services based on selected vendor and search query
  const vendorFilteredServices = selectedVendor === 'all'
    ? hotelsServices
    : hotelsServices.filter(service => service.vendor_id === selectedVendor);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Homes & Stays Management</h1>
          <p className="text-sm text-gray-500 mt-1">Manage and review all hotels</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <SearchBar
              placeholder="Search hotels by title, description, or vendor..."
      ...
      <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
        <div className="border-b border-gray-100 px-5 py-3">
          <h3 className="text-sm font-semibold text-gray-900">All Stays</h3>
