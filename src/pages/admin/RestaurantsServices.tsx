...
export function RestaurantsServices() {
  ...
  // Filter services to only show Restaurants
  const restaurantsServices = services.filter(service =>
    service.service_categories?.name?.toLowerCase() === 'restaurants'
  );

  const pendingServices = restaurantsServices.filter(service => service.status === 'pending');
  const approvedServices = restaurantsServices.filter(service => service.status === 'approved');
  const rejectedServices = restaurantsServices.filter(service => service.status === 'rejected');

  // Filter services based on selected vendor and search query
  const vendorFilteredServices = selectedVendor === 'all'
    ? restaurantsServices
    : restaurantsServices.filter(service => service.vendor_id === selectedVendor);
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <SearchBar
              placeholder="Search restaurants by title, description, or vendor..."
