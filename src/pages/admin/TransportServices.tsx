...
export function TransportServices() {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
          <div className="border-b border-gray-100 px-5 py-3">
            <h3 className="text-sm font-semibold text-gray-900">Transport Management</h3>
          </div>
          <div className="p-5">
  ...
  // Filter services to only show Transport
  const transportServices = services.filter(service =>
    service.service_categories?.name?.toLowerCase() === 'transport'
  );

  const pendingServices = transportServices.filter(service => service.status === 'pending');
  const approvedServices = transportServices.filter(service => service.status === 'approved');
  const rejectedServices = transportServices.filter(service => service.status === 'rejected');

  // Filter services based on selected vendor and search query
  const vendorFilteredServices = selectedVendor === 'all'
    ? transportServices
    : transportServices.filter(service => service.vendor_id === selectedVendor);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage and review all transport services</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            <SearchBar
              placeholder="Search transport services by title, description, or vendor..."
      ...
      <div className="bg-white shadow-sm rounded-xl border border-gray-200 overflow-hidden">
        <div className="border-b border-gray-100 px-5 py-3">
          <h3 className="text-sm font-semibold text-gray-900">All Transport</h3>
                  ...
                  <tr key={service.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      ...
                      <button
                        ...
                        title="Delete transport service"
                      >
          ...
          <div className="p-5">
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Transport Service</th>
